...
require __DIR__ . '/../auth/common/auth_guard.php'; // ✅ fixed
...
if ($view === 'accounts') {
  // Opening + income - expense in date range
  $accSql = "
    SELECT
      a.local_account_id,
      a.account_name,
  ...
  ";
  // args for join + user filter: [$uid,$from,$to,$uid]
  $st = $pdo->prepare($accSql); $st->execute([$uid,$from,$to,$uid]);
  $rows = $st->fetchAll(PDO::FETCH_ASSOC);
...
<?php if ($view === 'monthly'): ?>
  ...
  <table border="1" cellpadding="6">
    ...
    <?php $sumInc=0; $sumExp=0; foreach($rows as $r): $sumInc+=(float)$r['income']; $sumExp+=(float)$r['expense']; ?>
      <tr>
        <td><?= h($r['ym']) ?></td>
        <td style="text-align:right"><?= number_format((float)$r['income'],2) ?></td>
        <td style="text-align:right"><?= number_format((float)$r['expense'],2) ?></td>
        <td style="text-align:right"><?= number_format((float)$r['income'] - (float)$r['expense'],2) ?></td>
      </tr>
    <?php endforeach; ?>
    <tr>
      <th>Total</th>
      <th style="text-align:right"><?= number_format($sumInc,2) ?></th>
      <th style="text-align:right"><?= number_format($sumExp,2) ?></th>
      <th style="text-align:right"><?= number_format($sumInc-$sumExp,2) ?></th>
    </tr>
  </table>
<?php endif; ?>

<?php if ($view === 'category'): ?>
  <h3>By Category (<?= h($from) ?> → <?= h($to) ?>) 🏷️</h3>
  ...
  <table border="1" cellpadding="6">
    ...
    <?php endforeach; ?>
    <tr><th colspan="2">Grand Total</th><th style="text-align:right"><?= number_format($sum,2) ?></th></tr>
  </table>
<?php endif; ?>
  ...
  <table border="1" cellpadding="6">
    ...
    <?php $totOpen=0;$totInc=0;$totExp=0;$totBal=0; foreach($rows as $r):
      ...
      <tr>
        ...
        <td style="text-align:right"><?= number_format((float)$r['opening_balance'],2) ?></td>
        <td style="text-align:right"><?= number_format((float)$r['inc_amt'],2) ?></td>
        <td style="text-align:right"><?= number_format((float)$r['exp_amt'],2) ?></td>
        <td style="text-align:right"><?= number_format($bal,2) ?></td>
      </tr>
    <?php endforeach; ?>
    <tr>
      <th>Total</th>
      <th style="text-align:right"><?= number_format($totOpen,2) ?></th>
      <th style="text-align:right"><?= number_format($totInc,2) ?></th>
      <th style="text-align:right"><?= number_format($totExp,2) ?></th>
      <th style="text-align:right"><?= number_format($totBal,2) ?></th>
    </tr>
  </table>
<?php endif; ?>
