...
/** Build URL to /public/<path> even if APP_BASE already ends with /public */
function app_public(string $path): string {
    $base = rtrim(APP_BASE ?? '', '/');          // e.g. /pfms
    if (substr($base, -7) === '/public') {       // sanitize if misconfigured
...
if ($uid <= 0) {
    header('Location: ' . app_public('login.php?err=login_required'));
    exit;
}
...
if ($msg !== '') {
    $low = strtolower($msg);
    $isError = (strpos($low, 'fail') !== false)
            || (strpos($low, 'crash') !== false)
            || (strpos($low, 'not connected') !== false)
...
<head>
  ...
  <link rel="stylesheet" href="<?= app_public('dashboard.css') ?>">
  <style>
    /* (styles unchanged) */
    ...
    @media(max-width:880px){.content-grid{grid-template-columns:1fr}.charts-section{grid-template-columns:1fr}}
  </style>
</head>
<body>
...
<div class="app-container">
  ...
  <aside class="sidebar">
    ...
    <nav class="nav-menu">
      <a href="<?= app_public('dashboard.php') ?>" class="nav-item"><i class="fas fa-home"></i><span>Dashboard</span></a>
      ...
      <a href="<?= app_public('sync.php') ?>" class="nav-item active"><i class="fas fa-sync-alt"></i><span>Sync</span></a>
    </nav>
    <div class="sidebar-footer">
      <a href="<?= app_public('logout.php') ?>" class="logout-link"><i class="fas fa-sign-out-alt"></i><span>Logout</span></a>
    </div>
  </aside>
  ...
  <main class="main-content">
    ...
    <div class="msg <?= $isError ? 'error' : 'success' ?>">
      <div class="msg-icon"><i class="fas <?= $isError ? 'fa-times-circle' : 'fa-check-circle' ?>"></i></div>
      <div style="flex:1">
        <h4 style="margin:0 0 4px 0;font-weight:700;<?= $isError ? 'color:#7f1d1d' : 'color:#065f46' ?>">
          <?= $isError ? 'Synchronization Failed' : 'Synchronization Completed!' ?>
        </h4>
    ...
    <div class="control-panel">
      <div class="control-header">
        ...
        <div style="display:flex;gap:12px">
          <!-- IMPORTANT: POST straight to /api/run_sync.php (no /public here) -->
          <form method="post" action="<?= rtrim(APP_BASE, '/') ?>/api/run_sync.php" id="syncForm">
      ...
      <div class="progress-wave" id="progressWave"><div class="progress-wave-fill"></div></div>
    ...
    <!-- (Everything below unchanged: stats, charts, lists, tips, quick actions) -->
    <!-- Statistics Grid -->
    <div class="stats-grid">
      <div class="stat-card"><div class="stat-icon"><i class="fas fa-check-double"></i></div>
        <div class="stat-value"><?= $syncedTxn ?></div><div class="stat-label">Synced Transactions</div>
        <div class="stat-trend" style="color:#10b981"><i class="fas fa-arrow-up"></i><span>Successfully synchronized</span></div>
      </div>
      <div class="stat-card"><div class="stat-icon" style="background:linear-gradient(135deg,#f59e0b,#d97706);box-shadow:0 10px 30px rgba(245,158,11,.3)"><i class="fas fa-hourglass-half"></i></div>
        <div class="stat-value" style="color:#f59e0b"><?= $pendingTxn ?></div><div class="stat-label">Pending Sync</div>
        <div class="stat-trend" style="color:#f59e0b"><i class="fas fa-clock"></i><span>Waiting for sync</span></div>
      </div>
      <div class="stat-card"><div class="stat-icon" style="background:linear-gradient(135deg,#ef4444,#dc2626);box-shadow:0 10px 30px rgba(239,68,68,.3)"><i class="fas fa-exclamation-triangle"></i></div>
        <div class="stat-value" style="color:#ef4444"><?= $conflictTxn ?></div><div class="stat-label">Sync Conflicts</div>
        <div class="stat-trend" style="color:#ef4444"><i class="fas fa-exclamation-circle"></i><span>Require attention</span></div>
      </div>
      <div class="stat-card"><div class="stat-icon" style="background:linear-gradient(135deg,#3b82f6,#2563eb);box-shadow:0 10px 30px rgba(59,130,246,.3)"><i class="fas fa-database"></i></div>
        <div class="stat-value" style="color:#3b82f6"><?= $totalItems ?></div><div class="stat-label">Total Items</div>
        <div class="stat-trend" style="color:#3b82f6"><i class="fas fa-list"></i><span>Health: <?= $syncHealth ?>%</span></div>
      </div>
    </div>
    ...
    <div class="charts-section">
      <div class="chart-card">
        <div class="chart-header">
          <div><h3 style="margin:0;font-size:1.25rem;font-weight:700">Sync Status Distribution</h3>
            <p style="margin:4px 0 0 0;color:#6b7280;font-size:.875rem">Visual breakdown of all transactions</p></div>
          <span class="chart-badge">Live Data</span>
        </div>
        <div style="height:280px;position:relative"><canvas id="syncChart"></canvas></div>
      </div>
      <div class="chart-card">
        <div class="chart-header">
          <div><h3 style="margin:0;font-size:1.25rem;font-weight:700">Data Sync Progress</h3>
            <p style="margin:4px 0 0 0;color:#6b7280;font-size:.875rem">Accounts and Categories status</p></div>
          <span class="chart-badge">Updated</span>
        </div>
        <div style="height:280px;position:relative"><canvas id="dataChart"></canvas></div>
      </div>
    </div>
    ...
    <div class="content-grid">
      ...
      <div style="display:flex;flex-direction:column;gap:20px">
        ...
        <div class="card">
          <div class="card-header">
            <div><h3 style="margin:0 0 4px 0;font-size:1.25rem;font-weight:700"><i class="fas fa-clock" style="color:#f59e0b"></i> Recent Pending</h3>
              <p style="margin:0;color:#6b7280;font-size:.875rem">Items waiting to be synchronized</p></div>
          ...
          <?php else: foreach($recentPending as $row): ?>
        ...
        <div class="card">
          <div class="card-header">
            <div><h3 style="margin:0 0 4px 0;font-size:1.25rem;font-weight:700"><i class="fas fa-exclamation-triangle" style="color:#ef4444"></i> Sync Conflicts</h3>
              <p style="margin:0;color:#6b7280;font-size:.875rem">Items with synchronization issues</p></div>
          ...
          <div class="transaction-card" style="border-color:rgba(239,68,68,.3)">
            <div class="txn-content">
              <div class="txn-icon-circle conflict"><i class="fas fa-exclamation-triangle"></i></div>
              <div class="txn-info">
                <h4><?= h($row['category_name'] ?? 'Uncategorized') ?></h4>
                <p><i class="fas fa-wallet"></i> <?= h($row['account_name'] ?? 'Unknown') ?></p>
              </div>
      ...
      <div style="display:flex;flex-direction:column;gap:20px">
        <!-- Sync Details -->
        <div class="card">
          <h3 style="margin:0 0 20px 0;font-size:1.25rem;font-weight:700"><i class="fas fa-info-circle" style="color:#667eea"></i> Sync Details</h3>
          <div class="detail-row"><span style="color:#374151;font-weight:600"><i class="fas fa-wallet"></i> Accounts</span>
            <span style="font-weight:800;color:#111827"><?= $accSynced ?> / <?= $accSynced + $accNoSrv ?></span></div>
          <div class="detail-row"><span style="color:#374151;font-weight:600"><i class="fas fa-tags"></i> Categories</span>
            <span style="font-weight:800;color:#111827"><?= $catSynced ?> / <?= $catSynced + $catNoSrv ?></span></div>
          <div class="detail-row"><span style="color:#374151;font-weight:600"><i class="fas fa-exchange-alt"></i> Transactions</span>
            <span style="font-weight:800;color:#111827"><?= $syncedTxn ?> / <?= $totalItems ?></span></div>
          ...
          <div style="padding:14px;background:#f9fafb;border-radius:12px;margin-bottom:10px">
            <p style="margin:0 0 6px 0;color:#6b7280;font-size:.875rem"><i class="fas fa-database"></i> Oracle Version</p>
            <p style="margin:0;font-weight:700;color:#111827"><?= h($oracleStatus['oracle_version'] ?? 'n/a') ?></p>
          </div>
          <div style="padding:14px;background:#f9fafb;border-radius:12px">
            <p style="margin:0 0 6px 0;color:#6b7280;font-size:.875rem"><i class="fas fa-server"></i> Connection Info</p>
        ...
        <!-- Overview -->
        <div class="card">
          <h3 style="margin:0 0 20px 0;font-size:1.25rem;font-weight:700"><i class="fas fa-chart-pie" style="color:#667eea"></i> Sync Overview</h3>
        ...
        <!-- Tips -->
        <div class="tips-card">
          <h3 style="margin:0 0 16px 0;color:#92400e;display:flex;align-items:center;gap:8px"><i class="fas fa-lightbulb"></i> Pro Tips</h3>
          <div class="tip-item"><i class="fas fa-check"></i><span>Sync regularly to prevent data loss and ensure backup</span></div>
          <div class="tip-item"><i class="fas fa-check"></i><span>Resolve conflicts immediately to maintain data integrity</span></div>
          <div class="tip-item"><i class="fas fa-check"></i><span>Ensure stable internet connection before starting sync</span></div>
          <div class="tip-item"><i class="fas fa-check"></i><span>Monitor Oracle connection status regularly</span></div>
          <div class="tip-item"><i class="fas fa-check"></i><span>Check sync logs for any warnings or errors</span></div>
        </div>

        <!-- Quick Actions -->
        <div class="card">
          <h3 style="margin:0 0 16px 0;font-size:1.25rem;font-weight:700"><i class="fas fa-bolt" style="color:#667eea"></i> Quick Actions</h3>
          <a href="<?= app_public('dashboard.php') ?>" class="badge ok" style="display:block;text-align:center;padding:12px;margin-bottom:10px;text-decoration:none;cursor:pointer"><i class="fas fa-home"></i> Back to Dashboard</a>
          <a href="<?= APP_BASE ?>/app/transactions/index.php" class="badge warn" style="display:block;text-align:center;padding:12px;margin-bottom:10px;text-decoration:none;cursor:pointer"><i class="fas fa-exchange-alt"></i> View Transactions</a>
          <?php if ($conflictTxn > 0): ?>
          <a href="javascript:void(0)" class="badge err" style="display:block;text-align:center;padding:12px;text-decoration:none;cursor:pointer" onclick="alert('Conflict resolution feature coming soon!')"><i class="fas fa-exclamation-triangle"></i> Resolve Conflicts</a>
          <?php endif; ?>
        </div>
      </div>
    </div>
...
document.getElementById('syncForm')?.addEventListener('submit', function() {
  const btn=document.getElementById('syncBtn'), icon=document.getElementById('syncIcon'), text=document.getElementById('syncText');
  const progress=document.getElementById('progressWave'), progressText=document.getElementById('progressText');
  if (btn && !btn.disabled) {
    btn.disabled = true; btn.style.opacity = '0.7';
    if (icon) icon.style.animation = 'spin 1s linear infinite';
    if (text) text.textContent = 'Synchronizing...';
...
const chartOptions = {
  ...
  plugins:{ legend:{ position:'bottom', labels:{ padding:20, font:{ size:13, weight:'600' }, usePointStyle:true, pointStyle:'circle' } },
            tooltip:{ backgroundColor:'rgba(15,23,42,0.95)', padding:16, borderRadius:12, titleFont:{ size:14, weight:'700' }, bodyFont:{ size:13 } } }
};

// Sync Status Chart
new Chart(document.getElementById('syncChart'), {
  type:'doughnut',
  data:{ labels:['Synced','Pending','Conflicts'],
    datasets:[{ data:[<?= $syncedTxn ?>, <?= $pendingTxn ?>, <?= $conflictTxn ?>],
      backgroundColor:['#10b981','#f59e0b','#ef4444'], borderWidth:0, borderRadius:8, spacing:6, hoverOffset:15 }] },
  options:{ ...chartOptions, cutout:'70%', animation:{ animateScale:true, animateRotate:true } }
});

// Data Sync Chart
new Chart(document.getElementById('dataChart'), {
  type:'bar',
  data:{ labels:['Accounts','Categories'],
    datasets:[
      { label:'Synced',  data:[<?= $accSynced ?>, <?= $catSynced ?>],  backgroundColor:'#10b981', borderRadius:10, borderSkipped:false },
      { label:'Pending', data:[<?= $accNoSrv ?>,  <?= $catNoSrv ?>],   backgroundColor:'#f59e0b', borderRadius:10, borderSkipped:false }
    ]},
  options:{ ...chartOptions, scales:{
      y:{ beginAtZero:true, ticks:{ stepSize:1, font:{ size:12, weight:'600' } }, grid:{ color:'rgba(0,0,0,0.04)', drawBorder:false }, border:{ display:false } },
      x:{ grid:{ display:false }, ticks:{ font:{ size:13, weight:'600' } }, border:{ display:false } }
    },
...
document.querySelectorAll('.transaction-card').forEach(card=>{
  card.addEventListener('click',function(){ this.style.transform='translateX(10px)'; setTimeout(()=>{ this.style.transform='translateX(5px)'; },200); });
});

// Console info
console.log('PFMS Sync Center');
console.log('Sync Health: <?= $syncHealth ?>%');

// Optional auto-refresh hint
setTimeout(()=>{ if (<?= $pendingTxn ?> > 0 && navigator.onLine) { console.log('Consider refreshing to check sync status'); } }, 30000);
</script>
